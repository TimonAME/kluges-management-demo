...
const editingTodoWarning = ref(false)
...
const emits = defineEmits(['toggleTodoStatus', 'handleAddTodo', 'handleUpdateTodo', 'updateSearchQuery', "handleDeleteTodo"]);
...
const handleCloseEditForm = () => {
  editingTodo.value = null;
  editingTodoWarning.value = false;
};
...
<template>
  ...
  <div v-if="activeView === 'suche'" class="relative mb-6">
    <input
        v-model="searchQuery"
        @input="updateSearchQuery"
        type="text"
        class="w-full font-medium border-0 border-b border-transparent border-b-gray-300 hover:border-b-first_accent focus:ring-0 pl-10"
        placeholder="Suche nach Aufgaben"
    />
    <svg class="absolute left-2 top-1/2 transform -translate-y-1/2 w-5 h-5 stroke-gray-500" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  <div v-if="activeView === 'heute'" class="relative mb-6">
  <!-- "NO Todos text -->
    <div v-if="filteredTodos.length === 0" class="">
      <p class="text-lg font-medium text-secondary_text mt-4">Keine Todos heute zugewiesen</p>
    </div>
  </div>
      ...
      <div class="flex items-center gap-4 group/item">
        ...
        <div class="flex-1">
          ...
          <div class="flex flex-row items-center gap-4">
            ...
            <!-- Ersteller -->
            <div
                class="text-xs text-secondary_text flex-row gap-1 items-center"
                :class="activeView === 'eingang'  ? 'flex' : 'group-hover/item:flex hidden'"
            >
        ...
        <div class="flex flex-row gap-2" v-if="!todo.recentlyAdded">
          <svg v-if="activeView !== 'erledigt'" @click="openEditForm(todo)" class="w-6 h-6 stroke-icon_color invisible group-hover/item:visible hover:stroke-main" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  <!-- Add Task Form -->
  <div v-if="activeView !== 'erledigt' || activeView !== 'eingang'">
    <div
